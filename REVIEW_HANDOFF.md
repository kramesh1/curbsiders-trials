...
The automated ingestion and deterministic build are operational and reproducible. The public site currently contains show-note pearls, extracted evidence records, and source backlinks. It intentionally contains no model-suggested pearl→evidence link and no trial-screening summary presented as human-reviewed.
...
The historical 398 bulk “approved” pearl-link records were migrated to `auto_triaged`; the prior process was an automated high-confidence/direct-support rule with a 25-record spot check, not record-by-record human sign-off.
...
The scheduled wrapper requires a populated `.venv`; install the launchd/cron job separately using [automation/README.md](automation/README.md). Public feedback remains hidden until the Worker/D1 endpoint is deployed and configured.

## Known curation debt (not silently “fixed”)
